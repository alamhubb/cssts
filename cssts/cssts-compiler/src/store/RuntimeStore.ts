...
export class RuntimeStore {
    ...
    static getRuntimeData(name: string): RuntimeAtomData | undefined {
        ...
        return runtimeMap.get(name) as RuntimeAtomData | undefined
    }
    ...
    static getAtomGroup(name: string): 'atom' | 'group' | 'pseudo' | 'classGroup' | undefined {
        const runtimeMap: Map<string, RuntimeAtomData> | null = RuntimeStore._runtimeMap
        if (runtimeMap == null) return undefined
        const data: RuntimeAtomData | undefined = runtimeMap.get(name) as RuntimeAtomData | undefined
        return data == null ? undefined : data.group
    }
    ...
    static getAllAtomNames(filterGroup?: 'atom' | 'group' | 'pseudo' | 'classGroup'): string[] {
        ...
        const names: string[] = Array.from(runtimeMap.keys()) as string[]
        if (filterGroup == null) {
            return names
        }
        ...
        for (let i = 0; i < names.length; i++) {
            ...
            const data: RuntimeAtomData | undefined = runtimeMap.get(name) as RuntimeAtomData | undefined
            if (data != null && data.group === filterGroup) {
                result.push(name)
            }
        }
        return result
