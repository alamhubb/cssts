...
export const EXPECTED_CSSTS_LANGUAGE_SERVER_METADATA: LanguageServerMetadata = {
...
export function resolveLanguageServerMetadata(initializationOptions: unknown): LanguageServerMetadata {
  ...
  if (!initMetadata && !envMetadata) {
    throw new Error('CSSTS language server requires qin.languageServer initialization metadata or QIN_LSP_* environment metadata')
  }
  if (initMetadata && envMetadata) {
  ...
  assertCompleteMetadata(metadata, 'CSSTS language server metadata')
  assertSameMetadata(metadata, EXPECTED_CSSTS_LANGUAGE_SERVER_METADATA, 'CSSTS language server metadata', 'CSSTS language server expected metadata')
  return metadata
}
