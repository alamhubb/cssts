import {
  EXPECTED_CSSTS_LANGUAGE_SERVER_METADATA,
  resolveLanguageServerMetadata,
} from '../cssts-language-server/src/LanguageServerMetadata'
...
function main() {
  withCleanEnv(() => {
    const metadata = resolveLanguageServerMetadata({
      qin: {
        languageServer: EXPECTED_CSSTS_LANGUAGE_SERVER_METADATA,
      },
    })
    assert(metadata.sourceExtension === '.cssts', 'sourceExtension from initialization metadata')
    assert(metadata.compilerPackage === 'cssts-compiler', 'compilerPackage from initialization metadata')
  })
  ...
  withCleanEnv(() => {
    ...
    process.env.QIN_LSP_COMPILER_PACKAGE = 'cssts-compiler'
    const metadata = resolveLanguageServerMetadata({})
  ...
  console.log('CSSTS language server metadata smoke passed')
}
