...
const projectName = 'cssts-qin-managed-smoke'
const tempRoot = mkdtempSync(path.join(tmpdir(), 'create-cssts-smoke-'))
...
try {
  ...
  assert.equal(
    result.status,
    0,
    `create-cssts scaffold failed\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`
  )

  assertFile('qin.config.js')
  assertFile('package.json')
  assertFile('README.md')
  assertFile(path.join('src', 'styles', 'styles.cssts'))
