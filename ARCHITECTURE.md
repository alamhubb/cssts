...
`cssts-compiler` 提供 DTS 生成功能，为原子类生成 TypeScript 类型定义文件，实现 IDE 智能提示。

### 核心功能

- 生成全局常量声明（`declare const displayFlex`）
- 用户输入 `display` 即可获得所有 `display*` 原子类的提示
- 支持单文件和分文件两种模式
...
generateDtsFiles({
  ...
  splitFiles: false,              // 是否拆分文件（默认 false）
  verbose: true,                  // 是否打印日志
});
```
...
**优点**：
- 简单可靠，无路径问题
- TypeScript 性能无差异（类型数量相同）
- 推荐用于生产环境

#### 分文件模式（`splitFiles: true`）

按属性拆分为多个文件，使用 `/// <reference>` 引入：

```
target/cssts-dts/
├── package.json
├── index.d.ts          # reference 引入所有分文件
├── width.d.ts          # width 相关原子类
├── height.d.ts         # height 相关原子类
├── colors.d.ts         # 颜色相关原子类
├── keywords.d.ts       # 关键字原子类
└── ...                 # 其他属性文件
```
...
**分类规则**：
- 有数值的属性 → 单独文件（如 `width.d.ts`）
- 无数值但有颜色类型 → `colors.d.ts`
- 无数值无颜色 → `keywords.d.ts`
...
📁 生成分文件版本...
   ✅ 生成 52 个数值属性文件
   ✅ 生成 colors.d.ts (467 个原子类)
   ✅ 生成 keywords.d.ts (309 个原子类)
   ✅ 生成索引文件: index.d.ts
...
interface DtsGenerateResult {
  ...
  atomCount: number;    // 原子类总数
  stats: {
    totalAtoms: number;     // 原子类总数
    propertyCount: number;  // CSS 属性数
    categoryCount: number;  // 单位类型数
  };
}
```
...
1. **性能无差异**：TypeScript 编译器会把所有 `.d.ts` 加载到内存，类型数量相同则性能相同
2. **更简单可靠**：无 reference 路径问题
3. **29754 个原子类**：对 TypeScript 来说不算大，单文件完全能处理
4. **用户无需关心**：这是自动生成的文件，用户不需要阅读

**为什么保留分文件选项？**

1. **开发调试**：便于查看特定属性的原子类
2. **问题排查**：可以快速定位某个属性的类型定义
3. **文档参考**：分文件结构更清晰
